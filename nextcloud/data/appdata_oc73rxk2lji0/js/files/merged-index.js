		...
		if ($uploadEl.exists()) {
			...
			this.fileUploadParam = {
				...
				start: function(e) {
					...
					//hide the agenttip otherwise it covers the progress bar
					$('#upload').agenttip('hide');
...
(function() {
	...
	FileActions.prototype = {
		...
		_renderInlineAction: function(actionSpec, isDefault, context) {
			...
			$actionEl.agenttip({placement:'top'});
			return $actionEl;
		},
...
(function() {
	...
	FileList.prototype = {
		...
		initialize: function($el, options) {
			...
			$('#view-toggle').agenttip({placement: 'bottom', trigger: 'hover'});
		...
		_createRow: function(fileData, options) {
			...
			if (fileData.extraData) {
				...
				nameSpan.agenttip({placement: 'top'});
			}
		...
		rename: function(oldName) {
			...
			form.submit(function(event) {
				...
				try {
					...
					input.agenttip({placement: 'right', trigger: 'manual'});
					input.agenttip('fixTitle');
					input.agenttip('show');
		...
		_renderNewButton: function() {
			...
			$newButton.agenttip({'placement': 'bottom'});
...
(function() {
	var Files = {
		...
		initialize: function() {
			...
			$('#upload').agenttip({placement:'right'});
	...
	var MainFileInfoDetailView = OCA.Files.DetailFileInfoView.extend(
		...
		_setupClipboard: function() {
			...
			clipboard.on('success', function(e) {
				...
				$el.agenttip('hide')
					...
					.agenttip('fixTitle')
					.agenttip({placement: 'bottom', trigger: 'manual'})
					.agenttip('show');
				_.delay(function() {
					$el.agenttip('hide');
		...
		render: function() {
			...
			if (this.model) {
				...
				this.$el.find('[title]').agenttip({placement: 'bottom'});
			} else {
				this.$el.empty();
			}
			this.delegateEvents();
		...
		_promptFileName: function($target) {
			...
			var checkInput = function () {
				...
				try {
					...
					$input.attr('title', error);
					$input.agenttip({placement: 'right', trigger: 'manual', 'container': '.newFileMenu'});
					$input.agenttip('fixTitle');
					$input.agenttip('show');
					$input.addClass('error');
				}
...
(function() {
	var OperationProgressBar = OC.Backbone.View.extend({
		...
		setProgressBarText: function(textDesktop, textMobile, title) {
			...
			$('#uploadprogressbar').agenttip({placement: 'bottom', container: '#uploadprogresswrapper'});
			if (title) {
				$('#uploadprogressbar').attr('data-original-title', title);
				$('#uploadprogresswrapper .agenttip-inner').text(title);
			}
